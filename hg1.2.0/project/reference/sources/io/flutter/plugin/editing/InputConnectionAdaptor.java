...
public class InputConnectionAdaptor extends BaseInputConnection implements ListenableEditingState.EditingStateWatcher {
    ...
    private ExtractedText getExtractedText(ExtractedTextRequest extractedTextRequest) {
        ...
        if (extractedTextRequest == null || (extractedTextRequest.flags & 1) == 0) {
            string = this.mEditable.toString();
        } else {
            string = this.mEditable;
        }
    ...
    private static int clampIndexToEditable(int i, Editable editable) {
        int iMax = Math.max(0, Math.min(editable.length(), i));
        if (iMax != i) {
            Log.d("flutter", "Text selection index was clamped (" + i + "->" + iMax + ") to remain in bounds. This may not be your fault, as some keyboards may select outside of bounds.");
        }
        return iMax;
    }
    ...
    private boolean handleHorizontalMovement(boolean z, boolean z2) {
        ...
        if (z) {
            iMin = Math.max(this.flutterTextUtils.getOffsetBefore(this.mEditable, selectionEnd), 0);
        } else {
            iMin = Math.min(this.flutterTextUtils.getOffsetAfter(this.mEditable, selectionEnd), this.mEditable.length());
        }
    ...
    public boolean performContextMenuAction(int i) {
        beginBatchEdit();
        boolean zDoPerformContextMenuAction = doPerformContextMenuAction(i);
        endBatchEdit();
    ...
    @Override // android.view.inputmethod.BaseInputConnection, android.view.inputmethod.InputConnection
    public boolean commitContent(InputContentInfo inputContentInfo, int i, Bundle bundle) throws IOException {
    ...
    private byte[] readStreamFully(InputStream inputStream, int i) throws IOException {
        int i2;
