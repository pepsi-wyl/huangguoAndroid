...
public final class LongSaturatedMathKt {
    ...
    public static final long m2005saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM1922toLongimpl = Duration.m1922toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m2004checkInfiniteSumDefinedPjuGub4(j, j2, jM1922toLongimpl);
        }
        if ((1 | (jM1922toLongimpl - 1)) == Long.MAX_VALUE) {
            return m2006saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM1922toLongimpl;
        return ((j ^ j3) & (jM1922toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m2004checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1908isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m2006saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM1879divUwyO8pc = Duration.m1879divUwyO8pc(j2, 2);
        long jM1922toLongimpl = Duration.m1922toLongimpl(jM1879divUwyO8pc, durationUnit);
        return (1 | (jM1922toLongimpl - 1)) == Long.MAX_VALUE ? jM1922toLongimpl : m2005saturatingAddNuflL3o(m2005saturatingAddNuflL3o(j, durationUnit, jM1879divUwyO8pc), durationUnit, Duration.m1911minusLRDsOJo(j2, jM1879divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        Duration.Companion companion = Duration.INSTANCE;
        return j < 0 ? companion.m1978getNEG_INFINITEUwyO8pc$kotlin_stdlib() : companion.m1977getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1929unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                Duration.Companion companion = Duration.INSTANCE;
                return Duration.m1912plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m1929unaryMinusUwyO8pc(infinityOfSign(j3));
        }
