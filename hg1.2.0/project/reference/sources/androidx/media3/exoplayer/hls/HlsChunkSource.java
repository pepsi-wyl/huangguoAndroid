...
class HlsChunkSource {
    ...
    public void getNextChunk(LoadingInfo loadingInfo, long j, List<HlsMediaChunk> list, boolean z, HlsChunkHolder hlsChunkHolder) {
        ...
        int i2 = iIndexOf;
        Pair<Long, Integer> nextMediaSequenceAndPartIndex = getNextMediaSequenceAndPartIndex(hlsMediaChunk, z2, playlistSnapshot, initialStartTimeUs2, j);
        ...
        if (this.cmcdConfiguration != null) {
            CmcdData.Factory factory = new CmcdData.Factory(this.cmcdConfiguration, this.trackSelection, Math.max(0L, j5), loadingInfo.playbackSpeed, CmcdData.Factory.STREAMING_FORMAT_HLS, !hlsMediaPlaylist.hasEndTag, loadingInfo.rebufferedSince(this.lastChunkRequestRealtimeMs), list.isEmpty());
            if (getIsMuxedAudioAndVideo()) {
                objectType2 = CmcdData.Factory.OBJECT_TYPE_MUXED_AUDIO_AND_VIDEO;
            } else {
            ...
            SegmentBaseHolder nextSegmentHolder2 = getNextSegmentHolder(hlsMediaPlaylist, j2, nextSegmentHolder.partIndex == -1 ? -1 : nextSegmentHolder.partIndex + 1);
