...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        if (objNewInstance != null) {
            objM497constructorimpl = Result.m497constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
            if (Result.m503isFailureimpl(objM497constructorimpl)) {
                objM497constructorimpl = null;
            }
            return (Function1) objM497constructorimpl;
        }
    ...
    private final void dumpCoroutinesSynchronized(PrintStream out) {
        ...
        try {
            ...
            for (CoroutineOwner coroutineOwner : SequencesKt.sortedWith(SequencesKt.filter(CollectionsKt.asSequence(debugProbesImpl.getCapturedCoroutines()), new Function1<CoroutineOwner<?>, Boolean>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesSynchronized$1$2
                ...
                out.print("\n\nCoroutine " + coroutineOwner.delegate + ", state: " + ((Intrinsics.areEqual(debugCoroutineInfoImpl.get_state(), DebugCoroutineInfoImplKt.RUNNING) && listEnhanceStackTraceWithThreadDumpImpl == listLastObservedStackTrace) ? debugCoroutineInfoImpl.get_state() + " (Last suspension stacktrace, not an actual stacktrace)" : debugCoroutineInfoImpl.get_state()));
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            objM497constructorimpl = Result.m497constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM497constructorimpl = Result.m497constructorimpl(ResultKt.createFailure(th));
        }
        ...
        StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM497constructorimpl;
